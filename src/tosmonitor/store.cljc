(ns tosmonitor.store
  ...
  This repo archives exactly ONE company (AerCap Holdings N.V., LEI
  549300SZYINBBLJQU475) -- unlike `commitledger.store`'s multi-application
  ...
  discipline (this repo's own journal happens to use a single-flattened-map
  variant of the quad-log shape rather than multiple `[entity attr value tx
  op]` tuples -- `tosmonitor.registry/fold-quad-log` targets the
  multi-tuple shape and is not wired to this variant; irrelevant here since
  this Store never reads the file at all). `demo-baseline`'s full text is a
  SHORT, hand-written representative excerpt (not the real, page-length
...
;; page-length) -- see ns docstring.

(defn demo-company []
  {:legal-name "AerCap Holdings N.V."
   :website    "https://www.aercap.com"})

(def ^:private demo-full-text
  "Terms of Use\nThe aercap.com website is operated by AerCap Holdings N.V. and its subsidiaries. By accessing or using this website you agree to be legally bound by these Terms of Use. These Terms of Use shall be governed by and construed in accordance with the laws of Ireland.")

(defn demo-baseline []
  {:full-text    demo-full-text
   :source-url   "https://www.aercap.com/terms-of-use"
   :retrieved-at "2026-07-10T07:20:00Z"
