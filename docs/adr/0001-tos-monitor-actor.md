...
This repository archives the publicly published Terms of Use of AerCap
Holdings N.V., per ADR-2607110300 -- a read-only reference archive. As
...
- **Company**: AerCap Holdings N.V., LEI 549300SZYINBBLJQU475, website
  `https://www.aercap.com`.
- **Baseline provenance** (real, from this repo's own `80-data/public/
  tos.journal.edn` -- note: this repo's own journal uses a
  single-flattened-map variant of the archive family's usual multi-tuple
  quad-log shape; `tosmonitor.store` never reads either variant off disk
  regardless, see its own ns docstring): source-url
  `https://www.aercap.com/terms-of-use`, retrieved-at
  `2026-07-10T07:20:00Z`, doc-type `:terms-of-service`.
- **Baseline full text**: a short, hand-written representative excerpt (not
  the real, page-length archived page), with a self-consistent SHA-256
